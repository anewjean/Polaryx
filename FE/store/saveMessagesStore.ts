...
import { getSaveMessages, addSaveMessage, deleteSaveMessage } from '@/apis/saveMessageApi'
...
export const useSaveMessagesStore = create<SaveMessagesState>((set, get) => ({
  ...
  remove: async (workspaceId: string, id: string, userId: string) => {
    try {
      ...
      // 로컬 상태 업데이트
      set((state: SaveMessagesState) => ({ items: state.items.filter(m => m.save_message_id !== id) }))
    } catch {
      set({ error: '삭제 실패' })
    }
  },
